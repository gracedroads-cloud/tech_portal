...
function generateEstimate({
  ...
  const parsedLaborHours = Number(laborHours);
  const parsedMileage = Number(mileage);
  if (!Number.isFinite(parsedLaborHours) || parsedLaborHours < 0 ||
      !Number.isFinite(parsedMileage) || parsedMileage < 0) {
    throw new Error('Labor hours and mileage must be finite, non-negative numbers.');
  }
  const safeLaborHours = parsedLaborHours;
  const safeMileage = parsedMileage;
